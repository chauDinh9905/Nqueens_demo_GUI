...
class NQueensApp:
    def __init__(self, root):
        self.root = root
        self.root.title("N-Queens Visualizer Pro")
        self.n = 4; self.steps = []; self.current_step = 0; self.is_playing = False
        
        # Setup UI
        callbacks = {'start': self.start_solve, 'reset': self.reset, 'prev': self.prev_step, 
                     'play': self.toggle_play, 'next': self.next_step, 'speed': lambda v: None}
    ...
    def start_solve(self):
        ...
        self.steps, solutions, exec_time = solver.solve_with_steps(algo)
        
        self.current_step = 0
        self.info_panel.update_solutions_list(solutions)
        self.info_panel.update_time(exec_time)
    ...
    def next_step(self):
        if self.current_step < len(self.steps) - 1:
            self.current_step += 1; self._update_display()
    ...
    def toggle_play(self):
        ...
        if self.is_playing: self._play_animation()
    ...
    def reset(self):
        self.is_playing = False; self.steps = []; self.current_step = 0
        self.board_canvas.clear(); self.info_panel.reset()
        self._set_btn_state('disabled')

    def _set_btn_state(self, state):
        for btn in [self.control_panel.reset_btn, self.control_panel.prev_btn, 
